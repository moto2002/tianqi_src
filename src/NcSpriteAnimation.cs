...
public class NcSpriteAnimation : NcEffectAniBehaviour
{
	...
	private void Start()
	{
		...
		else
		{
			...
			{
				this.m_Renderer.set_enabled(false);
				return;
			}
	...
	private int CalcPartLoopInfo(int nSeqIndex, ref int nLoopCount)
	{
		...
		while (i < Mathf.Min(nLoopCount, this.m_nLoopFrameCount - 1))
		{
			...
			{
				base.get_transform().get_parent().SendMessage("OnSpriteAnimationLoopStart", nLoopCount, 1);
			}
			i++;
			num2++;
		}
		if (0 < this.m_nLoopingCount && this.m_nLoopingCount <= nLoopCount)
		{
			...
			if (base.get_transform().get_parent() != null)
			{
				base.get_transform().get_parent().SendMessage("OnSpriteAnimationLoopEnd", nLoopCount, 1);
			}
	...
	private void SetIndex(int nSeqIndex)
	{
		if (this.m_Renderer != null)
		{
			...
			if (this.m_TextureType == NcSpriteAnimation.TEXTURE_TYPE.TileTexture)
			{
				...
				{
					this.m_Renderer.get_material().set_mainTextureOffset(mainTextureOffset);
					this.m_Renderer.get_material().set_mainTextureScale(this.m_size);
				}
			}
	...
	public bool UpdateFactoryMaterial()
	{
		...
		if (this.m_NcSpriteFactoryPrefab.GetComponent<Renderer>() == null || this.m_NcSpriteFactoryPrefab.GetComponent<Renderer>().get_sharedMaterial() == null || this.m_NcSpriteFactoryPrefab.GetComponent<Renderer>().get_sharedMaterial().get_mainTexture() == null)
		{
			return false;
		}
		...
		base.GetComponent<Renderer>().set_sharedMaterial(this.m_NcSpriteFactoryPrefab.GetComponent<Renderer>().get_sharedMaterial());
		return true;
	}
	...
	private void CreateBuiltInPlane(int nSelIndex)
	{
		...
		{
			if (base.get_gameObject().GetComponent<MeshFilter>() != null)
			{
				this.m_MeshFilter = base.get_gameObject().GetComponent<MeshFilter>();
			}
			else
			{
				this.m_MeshFilter = base.get_gameObject().AddComponent<MeshFilter>();
			}
		}
	...
	private bool UpdateMeshUVsByTileTexture(Rect uv)
	{
		...
		if (this.m_MeshFilter == null || this.m_MeshFilter.get_sharedMesh() == null)
		{
			return false;
		}
		if (this.m_MeshUVsByTileTexture == null)
		{
			for (int i = 0; i < this.m_MeshFilter.get_sharedMesh().get_uv().Length; i++)
			{
				if (this.m_MeshFilter.get_sharedMesh().get_uv()[i].x != 0f && this.m_MeshFilter.get_sharedMesh().get_uv()[i].x != 1f)
				{
					return false;
				}
				if (this.m_MeshFilter.get_sharedMesh().get_uv()[i].y != 0f && this.m_MeshFilter.get_sharedMesh().get_uv()[i].y != 1f)
				{
					return false;
				}
			}
			this.m_MeshUVsByTileTexture = this.m_MeshFilter.get_sharedMesh().get_uv();
		}
		...
		for (int j = 0; j < this.m_MeshUVsByTileTexture.Length; j++)
		{
			...
			{
				array[j].x = uv.get_x() + uv.get_width();
			}
			if (this.m_MeshUVsByTileTexture[j].y == 1f)
			{
				array[j].y = uv.get_y() + uv.get_height();
			}
		}
		this.m_MeshFilter.get_mesh().set_uv(array);
		return true;
	}
