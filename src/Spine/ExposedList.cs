...
namespace Spine
{
	...
	public class ExposedList<T> : IEnumerable<T>, IEnumerable
	{
		...
		public ExposedList(IEnumerable<T> collection)
		{
			...
			{
				this.Items = new T[collection2.get_Count()];
		...
		{
			int count = collection.get_Count();
			if (count == 0)
			{
				return;
			}
		...
		public ExposedList<TOutput> ConvertAll<TOutput>(Converter<T, TOutput> converter)
		{
			...
			{
				exposedList.Items[i] = converter.Invoke(this.Items[i]);
			}
		...
		private int GetLastIndex(int startIndex, int count, Predicate<T> match)
		{
			...
			{
				if (match.Invoke(this.Items[--num]))
				{
					return num;
				}
			}
		...
		public void Sort()
		{
			Array.Sort<T>(this.Items, 0, this.Count, Comparer<T>.get_Default());
			this.version++;
		}
