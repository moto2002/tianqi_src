...
public class ConsumeGiftUI : UIBase
{
	public static ConsumeGiftUI Instance;
	...
	protected override void InitUI()
	{
		...
		this.consumedNum = base.FindTransform("Consumed").FindChild("ConsumedNum").GetComponent<Text>();
		this.consumedSlider = base.FindTransform("Progress").FindChild("Slider").GetComponent<Slider>();
	...
	private void OnClickBtnGet(GameObject sender)
	{
		...
		case 1:
			...
			{
				ConsumeRechargeManager.Instance.SendGetAcPrizeReq(6, this.showingBoxValue);
			}
			break;
		case 2:
	...
	private void ReflashBoxList(Ac down)
	{
		...
		this.consumedNum.set_text("当前消费：" + curData.ToString());
	...
	private void SetSliderBarValue(int consumed, List<AcItemInfo> BoxListInfo)
	{
		int count = BoxListInfo.get_Count();
		if (consumed >= BoxListInfo.get_Item(0).targetVal)
		{
			this.consumedSlider.set_value(1f);
			return;
		}
		for (int i = count - 1; i >= 0; i--)
		{
			if (consumed <= BoxListInfo.get_Item(i).targetVal)
			{
				if (i < count - 1)
				{
					int targetVal = BoxListInfo.get_Item(i + 1).targetVal;
					this.consumedSlider.set_value(((float)(count - 1 - i) + (float)(consumed - targetVal) / (float)(BoxListInfo.get_Item(i).targetVal - targetVal)) / (float)count);
				}
				else
				{
					this.consumedSlider.set_value((float)consumed / (float)BoxListInfo.get_Item(i).targetVal / (float)count);
				}
				return;
			}
		}
	...
	public void ShowRewardList(int targetValue)
	{
		...
		this.conditionText.set_text("累计消费" + text + "钻石可获得奖励:");
	...
	private void PlayCanGetSpine(bool isShow)
	{
		...
		{
			this.m_GetSpineTag = FXSpineManager.Instance.PlaySpine(2201, this.panelBtnGetSpine, "ConsumeGiftUI", 2000, null, "UI", 0f, 0f, 1f, 1f, false, FXMaskLayer.MaskState.None);
		}
