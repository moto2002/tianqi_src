...
public class DepthOfFieldOfUI : MonoBehaviour
{
	...
	private DepthOfFieldOfUI.BlurSampleCount blurSampleCount = DepthOfFieldOfUI.BlurSampleCount.High;

	private bool nearBlur;
	...
	{
		Camera expr_06 = base.GetComponent<Camera>();
		expr_06.set_depthTextureMode(expr_06.get_depthTextureMode() | 1);
		this.GetMaterials();
		if (this.focalTransform == null)
		{
			GameObject gameObject = new GameObject("FocusTransform");
			gameObject.get_transform().SetParent(CamerasMgr.CameraUI.get_transform());
			gameObject.get_transform().set_localPosition(new Vector3(0.46f, 0.11f, 1.35f));
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		else
		{
			...
			int num = (this.blurSampleCount != DepthOfFieldOfUI.BlurSampleCount.High && this.blurSampleCount != DepthOfFieldOfUI.BlurSampleCount.Medium) ? 11 : 17;
			if (this.highResolution)
			{
			...
			else
			{
				...
				Graphics.Blit(source, destination, this.dofHdrMaterial, (this.blurSampleCount != DepthOfFieldOfUI.BlurSampleCount.High) ? 12 : 18);
			}
		}
		if (renderTexture)
		{
